  ...
  return (
    <AnimatePresence mode="wait">
      <motion.div
        ...
        className={cn(
          'flex w-fit fixed top-4 inset-x-0 mx-auto border border-border-custom rounded-full bg-card-bg backdrop-blur-sm shadow-[0px_2px_3px_-1px_rgba(0,0,0,0.1),0px_1px_0px_0px_rgba(25,28,33,0.02),0px_0px_0px_1px_rgba(25,28,33,0.08)] z-[5000] px-4 py-3 items-center space-x-4',
          className,
        )}
      >
            ...
            {navItems.map((navItem: INavItem, idx: number) => (
              <span key={`link=${idx}`}>
                <Link
                  ...
                  className={cn(
                    'relative flex items-center space-x-1 text-foreground group',
                  )}
                >
                  ...
                  <span className="hidden sm:block text-sm/6 lg:text-base relative overflow-hidden">
                    ...
                    <span className="absolute inset-0 text-primary transition-transform transform translate-y-full group-hover:translate-y-0 duration-300 ease-in-out z-10">
                      {navItem.name}
                    </span>
                  </span>
                </Link>
